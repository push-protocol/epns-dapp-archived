...
import { Section, Item, ItemH, Anchor } from "components/SharedStyling";
...
function Navigation() {
  ...
  useEffect(() => {
    if (!loading) {
      ...
      // Set Primary List
      const primaryList = returnTransformedList(
        NavigationList.primary,
        GLOBALS.CONSTANTS.NAVBAR_SECTIONS.PRIMARY
      );

      // Set Secondary List
      const secondaryList = returnTransformedList(
      ...
      navList = Object.assign(
        navList,
        returnNavList(NavigationList.secondary, Object.keys(navList).length)
      );
    ...
    Object.entries(lists).forEach(([key, value]) => {
      ...
      if (section.hasOwnProperty("drilldown")) {
        ...
        Object.entries(section.drilldown).forEach(([drillkey, drillvalue]) => {
          ...
          drilldownModified[drillIdentifier].hasMenuLogic =
            drillvalue.hasMenuLogic;

          drilldownModified[drillIdentifier].id = drillkey;
          drilldownModified[drillIdentifier].parent =
            transformedList[identifier].id;
        ...
        if (key === "primary" || key === "secondary") {
          ...
          Object.entries(topSection).forEach(([key, value]) => {
            ...
            if (section.data.hasOwnProperty("drilldown")) {
              Object.entries(section.data.drilldown).forEach(
                ([drillkey, drillvalue]) => {
        ...
        if (key === "primary" || key === "secondary") {
          Object.keys(transformedMenuList[key]).forEach((sectionkey) => {
            ...
            if (section.id === activeParentId) {
              transformedMenuList[key][sectionkey].opened =
                !transformedMenuList[key][sectionkey].opened;
            ...
            if (section.id !== activeParentId) {
              if (section.active) {
                ...
                if (section.hasItems) {
                  ...
                  Object.keys(
                    transformedMenuList[key][sectionkey].data.drilldown
                  ).forEach((drillkey) => {
                    transformedMenuList[key][sectionkey].data.drilldown[
                      drillkey
                    ].active = false;
  ...
  const renderChildItems = (drilldown, opened, sectionID) => {
    ...
    let rendered = (
      <SectionGroup align="stretch" opened={opened} refresh={refresh}>
    ...
    >
      ...
      {navigationSetup && Object.keys(navigationSetup).length > 0 && (
        <>
          ...
          <Footer justify="flex-end" align="strecth">
            <Secondary
              align="stretch"
              justify="flex-end"
