...
export default class UsersDataStore {
  ...
  initUsersListenersAsync = async () => {
    ...
    if (
      !this.state.userMeta ||
      (this.state.userMeta && !this.state.userMeta.publicKeyRegistered)
    ) {
  ...
  getOwnerMetaAsync = async () => {
    ...
    return new Promise((resolve, reject) => {
      if (this.state.ownerMeta) {
        if (enableLogs)
          console.log(
            "getOwnerMetaAsync() [CACHED] --> %o",
            this.state.ownerMeta
          );
  ...
  getUsersCountAsync = async () => {
    return new Promise(async (resolve, reject) => {
      if (this.state.channelsCount === -1) {
        ...
        EPNSCoreHelper.getTotalNumberOfUsers(this.state.epnsReadProvider)
          .then((response) => {
  ...
  getUserMetaAsync = async () => {
    ...
    return new Promise((resolve, reject) => {
      if (this.state.userMeta) {
        if (enableLogs)
          console.log(
            "getUserMetaAsync() [CACHED] --> %o",
            this.state.userMeta
          );
        resolve(this.state.userMeta);
      } else {
        EPNSCoreHelper.getUserInfo(
          this.state.account,
