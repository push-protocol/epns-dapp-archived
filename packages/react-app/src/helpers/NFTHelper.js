  ...
  getTokenOfOwnerByIndex: async (user, index, contract) => {
    ...
    return new Promise((resolve, reject) => {
      contract
        ...
        .then((response) => {
          if (enableLogs)
            console.log("getTokenOfOwnerByIndex() --> %o", response);
          resolve(response.toNumber());
        })
        .catch((err) => {
      ...
      NFTHelper.getNFTBalance(owner, nftContract)
        .then(async (balance) => {
          ...
          for (let i = 0; i < balance; i++) {
            promises.push(
              NFTHelper.getTokenOfOwnerByIndex(owner, i, nftContract).then(
                async (tokenId) => {
          ...
          console.log(
            "🚀 ~ file: NFTHelper.js ~ line 83 ~ .then ~ NFTDetails",
            NFTDetails
          );
          resolve(NFTDetails);
        })
      ...
      NFTHelper.getClaimable(tokenId, rewardsContract)
        .then(async (claimable) => {
          await NFTHelper.getTokenMetadata(tokenId, nftContract).then(
            async (metadata) => {
              await NFTHelper.getOwnerOfTokenId(tokenId, nftContract).then(
                async (owner) => {
                  if (
                    tokenId != null &&
                    metadata != null &&
