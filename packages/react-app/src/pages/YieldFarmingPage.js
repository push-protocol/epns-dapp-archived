...
import { Section, Content, Item, ItemH, Span } from "components/SharedStyling";
...
function YieldFarmingPage() {
  ...
  const getPUSHPoolStats = React.useCallback(async () => {
    const pushPoolStats =
      await YieldFarmingDataStore.instance.getPUSHPoolStats();
  ...
  const getUserDataPUSH = React.useCallback(async () => {
    const userDataPUSH = await YieldFarmingDataStore.instance.getUserData(
      yieldFarmingPUSH
    );
  ...
  const getUserDataLP = React.useCallback(async () => {
    const userDataLP = await YieldFarmingDataStore.instance.getUserData(
      yieldFarmingLP
    );
          ...
          <>
            <Content themes={themes.yieldBg}>
              <ItemH margin="0px 15px 0px 15px" align="stretch">
                ...
                <StatsCard bg={themes.mainBg}>
                  <StatsHeading bg={themes.pushPriceBg}>
                    PUSH Price
                  </StatsHeading>
                  <StatsContent>
